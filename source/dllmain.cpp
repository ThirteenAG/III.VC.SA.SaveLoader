...
void III()
{
    ...
    struct psInitialize
    {
        void operator()(injector::reg_pack&)
        {
            ...
            if (bSkipIntro)
            {
                auto pattern = hook::pattern("C7 05 ? ? ? ? ? ? ? ? E9 ? ? ? ? 83 3D");
                injector::WriteMemory<uint8_t>(pattern.get(1).get<uint32_t>(6), 0x05, true); //0x582A75
            }
...
void VC()
{
    ...
    struct psInitialize
    {
        void operator()(injector::reg_pack&)
        {
            ...
            {
                auto pattern = hook::pattern("C7 05 ? ? ? ? ? ? ? ? E9 ? ? ? ? 83 3D");
                injector::WriteMemory<uint8_t>(pattern.get_first(6), 0x05, true); //0x5FFFAB
            }
...
DWORD WINAPI Init(LPVOID)
{
    ...
    char* str = iniReader.ReadString("MAIN", "CustomUserFilesDirectoryInGameDir", "0");
    strcpy(szCustomUserFilesDirectory, str);
